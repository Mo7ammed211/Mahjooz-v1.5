...
function _ddb_renderCatsList() {
  ...
  <div class="ddb-wrap">
    <div class="ddb-page-header">
      <div>
        <h2 class="ddb-page-title">🚗 قاعدة بيانات مندوبين التوصيل${window.ui_helpBtn('drivers_database')}</h2>
    ...
    <div class="ddb-entries-list">
      ...
      <div class="ddb-entry-row">
        ...
        <div class="ddb-entry-body">
          ...
          ${e.regionId ? (() => { const rn=(AppData.regions||[]).find(r=>r.id===e.regionId); return rn?`<div style="font-size:11px;margin-top:3px;"><span style="background:rgba(124,58,237,0.12);color:#7c3aed;border:1px solid rgba(124,58,237,0.25);border-radius:5px;padding:1px 7px;font-weight:700;">📍 ${escHtml(rn.name)}</span></div>`:''; })() : ''}
        </div>
  ...
  <div class="ddb-wrap">
    ...
    <div class="ddb-driver-hero">
      ...
      <div class="ddb-driver-hero-info">
        ...
        ${entry.regionId ? (() => { const rn=(AppData.regions||[]).find(r=>r.id===entry.regionId); return rn ? `<div style="margin-top:8px;"><span style="background:rgba(124,58,237,0.12);color:#7c3aed;border:1.5px solid rgba(124,58,237,0.3);border-radius:8px;padding:4px 12px;font-size:13px;font-weight:800;display:inline-block;">📍 المنطقة: ${escHtml(rn.name)}</span></div>` : ''; })() : ''}
...
function _ddb_entryModalHTML(catId,subcatId,entry=null) {
  ...
  <div style="padding:20px;display:flex;flex-direction:column;gap:14px;max-height:75vh;overflow-y:auto">
    ...
    <div class="form-group">
      <label class="form-label">📍 المنطقة الجغرافية التابعة <span style="font-size:11px;color:var(--text-muted);">(يستخدم في توجيه الطلبات)</span></label>
      <select class="form-control" id="ddb-entry-region">
        <option value="">— جميع المناطق (يستقبل من أي منطقة) —</option>
        ${(AppData.regions||[]).filter(r=>r.active!==false).map(r=>`<option value="${r.id}" ${entry?.regionId===r.id?'selected':''}>${escHtml(r.name)}</option>`).join('')}
      </select>
    </div>
    <div class="form-group">
...
window.ddb_submitEntry = async function(catId,subcatId,id=null) {
  ...
  const regionId    = document.getElementById('ddb-entry-region')?.value || null;
  const active      = id?(document.getElementById('ddb-entry-active')?.checked!==false):true;
  if(!name){toast('اسم المندوب مطلوب','error');return;}
  showLoader('جاري الحفظ...');
  try{
    const payload={name,phone:phone||'',vehicleType:vehicleType||'',vehicleNumber:vehicleNum||'',coverageAreas:coverage||'',notes:notes||'',catId,subcatId,regionId,linkedUserId:uid||null,active,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
    let newId=id;
