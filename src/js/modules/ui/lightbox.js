...
// ════════════════════════════════════════════════════════════
//  5) نظام تكبير الإعلانات (Ads Lightbox) وتتبع الإحصائيات
// ════════════════════════════════════════════════════════════
window.ph27_trackAdClick = async function(adId) {
  try {
    const ad = (AppData.ads || []).find(a => a.id === adId);
    if (ad) {
      ad.clicks = (ad.clicks || 0) + 1;
      if (typeof fsUpdate === 'function') {
        await fsUpdate('ads', adId, { clicks: firebase.firestore.FieldValue.increment(1) });
      }
    }
  } catch(e) {
...
window.ph27_trackAdImpression = async function(adId) {
  try {
    const ad = (AppData.ads || []).find(a => a.id === adId);
    if (ad) {
      ad.impressions = (ad.impressions || 0) + 1;
      if (typeof fsUpdate === 'function') {
        await fsUpdate('ads', adId, { impressions: firebase.firestore.FieldValue.increment(1) });
      }
    }
  } catch(e) {
...
window.ph27_trackAdImpressionOnce = function(adId) {
  window._trackedAdImpressions = window._trackedAdImpressions || new Set();
  if (!window._trackedAdImpressions.has(adId)) {
    window._trackedAdImpressions.add(adId);
    window.ph27_trackAdImpression(adId);
  }
};

window.ph27_openAdLightbox = function(adId) {
  const ad = (AppData.ads || []).find(a => a.id === adId);
  if (!ad) return;

  // Remove existing ad lightbox if any
  window.ph27_closeAdLightbox(true);

  // Mark impression and click when opening the lightbox
  window.ph27_trackAdImpressionOnce(ad.id);

  const container = document.createElement('div');
  container.id = 'ph27-ad-lightbox';
  container.className = 'ad-lightbox-overlay';

  const badgeHtml = ad.type === 'popup' ? '🔥 عرض منبثق' : ad.type === 'interstitial' ? '📱 عرض ملء الشاشة' : '⚡ إعلان مميز';
  
  // Escape HTML helper inside template
  const titleEsc = typeof escHtml === 'function' ? escHtml(ad.title) : ad.title;
  const descEsc = ad.description ? (typeof escHtml === 'function' ? escHtml(ad.description) : ad.description) : '';
  const urlEsc = ad.targetUrl ? (typeof escAttr === 'function' ? escAttr(ad.targetUrl) : ad.targetUrl) : '';

  container.innerHTML = `
    <div class="ad-lightbox-card" onclick="event.stopPropagation()">
      <button class="ad-lightbox-close-btn" onclick="ph27_closeAdLightbox()" aria-label="إغلاق">✕</button>
      
      <div class="ad-lightbox-img-wrapper">
        ${ad.imageBase64 
          ? `<img src="${ad.imageBase64}" class="ad-lightbox-img" alt="${titleEsc}">` 
          : `<div style="padding: 60px; font-size: 48px; text-align: center;">📢</div>`}
      </div>
      
      <div class="ad-lightbox-body">
        <span class="ad-lightbox-badge">${badgeHtml}</span>
        <h3 class="ad-lightbox-title">${titleEsc}</h3>
        ${descEsc ? `<p class="ad-lightbox-desc">${descEsc}</p>` : ''}
        
        <div class="ad-lightbox-footer">
          ${ad.targetUrl 
            ? `<a href="${urlEsc}" target="_blank" class="ad-lightbox-action-btn primary" onclick="ph27_trackAdClick('${ad.id}'); ph27_closeAdLightbox();">🔗 زيارة الرابط</a>` 
            : ''}
          ${ad.serviceId 
            ? `<button class="ad-lightbox-action-btn primary" onclick="ph27_closeAdLightbox(); if(typeof bookService === 'function') bookService('${ad.serviceId}');">🛒 اطلب الآن</button>` 
            : ''}
          <button class="ad-lightbox-action-btn secondary" onclick="ph27_closeAdLightbox()">إغلاق</button>
        </div>
      </div>
    </div>
  `;
  ...
  container.addEventListener('click', () => ph27_closeAdLightbox());
  ...
  // Handle escape key
  window.addEventListener('keydown', ph27_onAdLightboxKey);
};

window.ph27_closeAdLightbox = function(immediate = false) {
  const el = document.getElementById('ph27-ad-lightbox');
  if (!el) return;

  window.removeEventListener('keydown', ph27_onAdLightboxKey);
...
function ph27_onAdLightboxKey(e) {
  if (e.key === 'Escape') {
    window.ph27_closeAdLightbox();
  }
}

// ─── Slider Navigation Helpers ───
window.ph27_slideAdNext = function(sliderId) {
  const slider = document.getElementById(sliderId);
  if (!slider || typeof slider._goTo !== 'function') return;
  if (slider._autoSlideInterval) {
    clearInterval(slider._autoSlideInterval);
    slider._autoSlideInterval = null;
  }
  slider._goTo(slider._getCur() + 1);
};

window.ph27_slideAdPrev = function(sliderId) {
  const slider = document.getElementById(sliderId);
  if (!slider || typeof slider._goTo !== 'function') return;
  if (slider._autoSlideInterval) {
    clearInterval(slider._autoSlideInterval);
...
console.log('[Phase 27] Ads Lightbox + stats tracking loaded.');
