  ...
  const explorerRows = filteredItems.map(item => {
    ...
    <tr class="ph46-table-row">
      ...
      <td>
        ...
        ${(item.discountPct && item.discountPct > 0 && item.discountActive) ? `
        <div style="margin-top:4px;">
          <span class="ph46-badge" style="background:#dc262620;color:#dc2626;border:1px solid #dc262640;font-size:10.5px;padding:2px 6px;border-radius:5px;">🏷️ خصم ${item.discountPct}% · ${item.discountDays || 0} يوم</span>
        </div>` : ''}
      </td>
      <td>
...
function ph46_renderItemsList() {
  ...
  const rows = items.map(item => {
    ...
    const skuBadge = item.sku
      ? `<span style="display:inline-block;font-family:monospace;font-size:11px;font-weight:700;background:rgba(139,92,246,0.12);color:#8b5cf6;border:1px solid rgba(139,92,246,0.25);border-radius:5px;padding:2px 7px;margin-top:3px;letter-spacing:0.5px;">#${escHtml(item.sku)}</span>`
      : '';
    const stockChip = typeof ph48_stockChipHtml === 'function' ? ph48_stockChipHtml(item) : '';
        ...
        <div class="ph46-table-item-cell">
          ...
          <div>
            ...
            <div style="display:flex; align-items:center; gap:6px; margin-top:4px; flex-wrap:wrap;">
              ${skuBadge}
              ${stockChip}
              ${item.alertBadge?.text ? `<span class="ia-stock-chip" style="background:rgba(59,130,246,0.12); color:#3b82f6; border:1px solid rgba(59,130,246,0.25);">🔔 ${escHtml(item.alertBadge.text)}</span>` : ''}
            </div>
          </div>
        </div>
      </td>
...
window.ph46_saveItem = async function(sectionId) {
  ...
  const govId = document.getElementById('ph46-item-gov')?.value || null;
  ...
  // الخصومات
  const discountActive = document.getElementById('ph46-discount-toggle')?.checked || false;
  const discountPct = parseFloat(document.getElementById('ph46-discount-pct')?.value) || 0;
  ...
  try {
    const itemData = {
      ...
      discountLabel: discountActive ? discountLabel : '',
      discountStartDate: discountActive
        ? (editId && allItems.find(i => i.id === editId)?.discountActive && allItems.find(i => i.id === editId)?.discountStartDate
            ? allItems.find(i => i.id === editId).discountStartDate
            : new Date().toISOString())
        : null,
      mainImage: window.__ph46_mainImg || null,
      images: window.__ph46_imgs || [],
      videoUrl: null, // يتم تحديثه بعد الرفع إن وُجد فيديو
    ...
    if (typeof ph48_collectAlertData === 'function') {
      ...
      itemData.stockStatus = alertData.stockStatus || 'available';
      itemData.alertBadge  = alertData.alertBadge  || null;
    }

    // رفع الفيديو إن اختار المستخدم فيديواً
    const vidInput = document.getElementById('ph46-item-video-input');
    if (vidInput && vidInput.files && vidInput.files[0] && typeof window.mUpload_uploadVideo === 'function') {
      const vidFile = vidInput.files[0];
      const check = typeof window.mUpload_validateVideo === 'function' ? window.mUpload_validateVideo(vidFile) : { valid: true };
      if (!check.valid) { toast(check.error, 'error'); hideLoader(); return; }
      window.mUpload_showProgressToast('🎬 جاري رفع الفيديو...');
      const ext = vidFile.name.split('.').pop().toLowerCase();
      const docId = editId || `new_${Date.now()}`;
      itemData.videoUrl = await window.mUpload_uploadVideo(
        vidFile,
        `videos/catalog/${docId}_${Date.now()}.${ext}`,
        (pct) => window.mUpload_updateProgressToast(pct, `🎬 ${pct}%`)
      );
      window.mUpload_hideProgressToast();
    } else if (editId) {
      // احتفظ بالفيديو الموجود إن لم يتم اختيار فيديو جديد
      const existingItem = allItems.find(i => i.id === editId);
      itemData.videoUrl = existingItem?.videoUrl || null;
    }
  ...
  const modalHtml = `
    ...
    <div class="ph46-modal-body-scroll" style="max-height: 75vh; overflow-y: auto; padding: 10px 5px; direction: rtl;">
      <div style="display:grid; grid-template-columns: 1fr; gap:16px;">
        ...
        <!-- قسم حالة التوفر والتنبيهات — يظهر مبكراً لسهولة الوصول -->
        ${typeof ph48_adminSectionHtml === 'function' ? `
        <div style="background:linear-gradient(135deg,rgba(99,102,241,0.06),rgba(139,92,246,0.04)); border:1px solid rgba(99,102,241,0.3); border-radius:12px; padding:16px;">
        ...
        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; padding: 16px;">
          ...
          <!-- قسم الخصومات -->
          <div style="margin-top:16px; padding:14px; background:rgba(220,38,38,0.06); border:1px solid rgba(220,38,38,0.2); border-radius:10px;">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
              <label style="font-size:14px; font-weight:700; color:#dc2626; display:flex; align-items:center; gap:6px;">
                🏷️ تفعيل الخصم على هذا العنصر
              </label>
              <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                <input type="checkbox" id="ph46-discount-toggle" onchange="ph46_toggleDiscountSection(this.checked)" style="width:18px; height:18px; cursor:pointer;">
                <span style="font-size:12px; color:var(--text-muted);">تفعيل الخصم</span>
              </label>
            </div>
            <div id="ph46-discount-fields" style="display:none;">
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                <div class="form-group">
                  <label class="form-label">نسبة الخصم (%)</label>
                  <input class="form-control" type="number" id="ph46-discount-pct" min="1" max="99" placeholder="مثال: 20">
                </div>
                <div class="form-group">
                  <label class="form-label">مدة الخصم (بالأيام)</label>
                  <input class="form-control" type="number" id="ph46-discount-days" min="1" placeholder="مثال: 7">
                </div>
              </div>
              <div class="form-group" style="margin-top:10px;">
                <label class="form-label">وصف فترة الخصم (اختياري)</label>
                <input class="form-control" type="text" id="ph46-discount-label" placeholder="مثال: عرض العيد الوطني، تخفيضات نهاية الموسم...">
              </div>
              <div style="font-size:11px; color:var(--text-muted); margin-top:8px; padding:6px 10px; background:rgba(0,0,0,0.1); border-radius:6px; line-height:1.5;">
                ⚠️ سيُحسب السعر المخفض تلقائياً للعملاء. مثال: إذا كان السعر 100 ريال والخصم 20% → السعر بعد الخصم = 80 ريال
              </div>
            </div>
          </div>
        </div>
        ...
        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; padding: 16px;">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="ph46-item-gov" onchange="ph_onFormGovChange('ph46-item-gov', 'ph46-item-region')">
              <option value="">-- اختر المحافظة (تغطية عامة إذا تركت فارغة) --</option>
              ${(AppData.deliveryGovernorates || []).filter(g => g.active !== false).map(g => `<option value="${g.id}">${escHtml(g.name)}</option>`).join('')}
            </select>
          </div>
          <div class="form-group" style="margin-top:12px;">
        ...
        <!-- القسم الخامس: إدارة الصور والفيديو -->
        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; padding: 16px;">
          <h4 style="margin-top:0; margin-bottom:12px; color:var(--primary); font-size:15px; border-bottom:1px solid var(--glass-border); padding-bottom:6px;">🎬 الصور والفيديو</h4>
          <div class="ph46-upload-zone" style="border: 2px dashed var(--glass-border); border-radius: 12px; padding: 24px; text-align: center; cursor: pointer; background: rgba(255,255,255,0.01); transition: all 0.2s;" onclick="document.getElementById('ph46-item-file-input').click()">
            ...
            <input type="file" id="ph46-item-file-input" multiple accept="image/*" style="display:none;" onclick="event.stopPropagation()" onchange="ph46_handleFileSelect(event)">
          </div>
          ...
          <div style="margin-top:16px; border-top:1px solid var(--glass-border); padding-top:14px;">
            <label class="form-label" style="display:flex;align-items:center;gap:8px;font-weight:700">
              🎬 فيديو المنتج/الخدمة
              <span style="font-size:10px;background:rgba(139,92,246,0.12);color:var(--primary);border:1px solid rgba(139,92,246,0.2);border-radius:6px;padding:1px 6px;font-weight:600">اختياري</span>
              <span style="font-size:10px;color:var(--text-muted);font-weight:400">MP4, WebM, MOV — حد أقصى 50 MB</span>
            </label>
            <div class="mu-drop-zone mu-video-zone" style="border:2px dashed rgba(13,148,136,0.4);border-radius:12px;cursor:pointer;background:rgba(13,148,136,0.02);padding:16px;text-align:center" onclick="document.getElementById('ph46-item-video-input').click()"
              ondragover="event.preventDefault();this.style.borderColor='#0d9488'"
              ondragleave="this.style.borderColor='rgba(13,148,136,0.4)'">
              <input type="file" id="ph46-item-video-input" accept=".mp4,.webm,.mov,.avi,.mkv,.3gp,.m4v,video/*,video/mp4,video/webm,video/quicktime" style="display:none;" onclick="event.stopPropagation()"
                onchange="if(typeof mUpload_previewVideo==='function')mUpload_previewVideo(this,'ph46-item-video-preview','ph46-item-video-prog')">
              <div id="ph46-item-video-preview">
                <span style="font-size:28px">🎬</span>
                <p style="margin:6px 0 0;font-size:12px;color:var(--text-muted)">انقر أو اسحب فيديو هنا (MP4, WebM, MOV)</p>
              </div>
              <div id="ph46-item-video-prog" style="display:none;margin-top:8px">
                <div style="height:4px;background:rgba(255,255,255,0.1);border-radius:4px;overflow:hidden">
                  <div id="ph46-item-video-prog-fill" style="height:100%;background:linear-gradient(90deg,#0d9488,#10b981);transition:width 0.3s"></div>
                </div>
                <div id="ph46-item-video-prog-text" style="font-size:11px;color:var(--text-muted);text-align:center;margin-top:4px"></div>
              </div>
            </div>
          </div>
        </div>
          <div id="ph46-media-manager-preview" style="display:none"></div>
      </div>
      ...
      <div style="display:grid; grid-template-columns: 1fr; gap:16px;">
        ...
        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; padding: 16px;">
          ...
          <div style="margin-top:16px; padding:14px; background:rgba(220,38,38,0.06); border:1px solid rgba(220,38,38,0.2); border-radius:10px;">
            <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px;">
              ...
              <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                <input type="checkbox" id="ph46-discount-toggle" ${item.discountActive ? 'checked' : ''} onchange="ph46_toggleDiscountSection(this.checked)" style="width:18px; height:18px; cursor:pointer;">
                <span style="font-size:12px; color:var(--text-muted);">تفعيل الخصم</span>
              </label>
            </div>
            <div id="ph46-discount-fields" style="display:${item.discountActive ? 'block' : 'none'};">
              <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                <div class="form-group">
                  <label class="form-label">نسبة الخصم (%)</label>
                  <input class="form-control" type="number" id="ph46-discount-pct" min="1" max="99" placeholder="مثال: 20" value="${item.discountPct || ''}">
                </div>
                <div class="form-group">
                  <label class="form-label">مدة الخصم (بالأيام)</label>
                  <input class="form-control" type="number" id="ph46-discount-days" min="1" placeholder="مثال: 7" value="${item.discountDays || ''}">
                </div>
              </div>
              <div class="form-group" style="margin-top:10px;">
                <label class="form-label">وصف فترة الخصم (اختياري)</label>
                <input class="form-control" type="text" id="ph46-discount-label" placeholder="مثال: عرض العيد الوطني، تخفيضات نهاية الموسم..." value="${escAttr(item.discountLabel || '')}">
              </div>
              ${item.discountStartDate ? `
              <div style="font-size:11px; color:#10b981; margin-top:8px; padding:6px 10px; background:rgba(16,185,129,0.08); border-radius:6px;">
                ✅ الخصم فعّال منذ: ${new Date(item.discountStartDate).toLocaleDateString('ar-SA')} · ينتهي بعد ${item.discountDays || 0} يوم
              </div>` : ''}
              <div style="font-size:11px; color:var(--text-muted); margin-top:8px; padding:6px 10px; background:rgba(0,0,0,0.1); border-radius:6px; line-height:1.5;">
                ⚠️ سيُحسب السعر المخفض تلقائياً للعملاء. مثال: إذا كان السعر 100 ريال والخصم 20% → السعر بعد الخصم = 80 ريال
              </div>
        ...
        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; padding: 16px;">
          ...
          <div class="form-group">
            ...
            <select class="form-control" id="ph46-item-gov" onchange="ph_onFormGovChange('ph46-item-gov', 'ph46-item-region')">
              <option value="">-- اختر المحافظة (تغطية عامة إذا تركت فارغة) --</option>
              ${(AppData.deliveryGovernorates || []).filter(g => g.active !== false).map(g => `<option value="${g.id}" ${g.id === item.govId ? 'selected' : ''}>${escHtml(g.name)}</option>`).join('')}
            </select>
          </div>
          <div class="form-group" style="margin-top:12px;">
            <label class="form-label">المنطقة الجغرافية التابعة</label>
            <select class="form-control" id="ph46-item-region">
              <option value="">-- اختر المنطقة --</option>
              ${(AppData.regions || []).filter(r => r.govId === item.govId && r.active !== false).map(r => `<option value="${r.id}" ${r.id === item.regionId ? 'selected' : ''}>${r.name}</option>`).join('')}
            </select>
          </div>
          <div class="form-group" style="margin-top:12px;">
            <label class="form-label">العنوان التفصيلي</label>
        ...
        <div style="background: rgba(255,255,255,0.02); border: 1px solid var(--glass-border); border-radius: 12px; padding: 16px;">
          ...
          <div style="margin-top:16px; border-top:1px solid var(--glass-border); padding-top:14px;">
            ...
            <div class="mu-drop-zone mu-video-zone" style="border:2px dashed rgba(13,148,136,0.4);border-radius:12px;cursor:pointer;background:rgba(13,148,136,0.02);padding:16px;text-align:center" onclick="document.getElementById('ph46-item-video-input').click()"
              ...
              <div id="ph46-item-video-preview">
                ${item.videoUrl
                  ? `<video src="${item.videoUrl}" class="mu-video-thumb" controls muted style="width:100%;max-height:200px;border-radius:8px;background:#000"></video>
