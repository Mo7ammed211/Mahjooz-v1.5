...
window.ph_offerPriceHtml = function (offer, fallbackHtml, currency) {
  ...
  return `<div class="ph-price-block ph-price-offer">
    <span class="ph-price-discounted">${disc.toLocaleString('ar-YE')} <span class="ph-price-cur">${cur}</span></span>
    ${orig ? `<span class="ph-price-original">${orig.toLocaleString('ar-YE')}</span>` : ''}
    ${save > 0 ? `<span class="ph-price-save">وفّر ${save.toLocaleString('ar-YE')}</span>` : ''}
  </div>`;
};
