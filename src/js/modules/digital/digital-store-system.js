...
window.ph45_formatBytes = function(bytes, decimals = 2) {
...
window.ph45_renderAdminDigitalStores = function() {
  ...
  if (activeStoreId) {
    ...
    const rows = filtered.map(p => {
      const isFileStore = store.storeType === 'file';
      const available = (AppData.digitalCodes||[]).filter(c=>c.digitalProductId===p.id&&c.status==='available').length;
      return `
      <tr>
        <td style="font-weight:700">
          <div>${p.name}</div>
          ${isFileStore && p.fileName ? `<div style="font-size:11px;color:var(--text-muted);font-weight:normal;margin-top:2px">📂 ${p.fileName} (${ph45_formatBytes(p.fileSize)})</div>` : ''}
        </td>
        ...
        <td>
          ${isFileStore ? `
            <button class="btn btn-sm" style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.3);color:#10b981"
              onclick="ph45_showUploadFileModal('${p.id}','${activeStoreId}')">
              📁 ${p.fileName ? 'تغيير الملف' : 'رفع ملف'}
            </button>
          ` : `
...
window.ph45_onStoreTypeChange = function(val) {
  const fileCatWrap = document.getElementById('dstore-file-cat-wrap');
  if (fileCatWrap) {
    fileCatWrap.style.display = val === 'file' ? 'block' : 'none';
  }
};
  ...
  openModal(`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="form-group">
        ...
        <select class="form-control" id="dstore-type" onchange="ph45_onStoreTypeChange(this.value)">
          <option value="code">🔑 أكواد رقمية</option>
          <option value="file">📂 ملفات رقمية</option>
        </select>
      </div>
      <div class="form-group" id="dstore-file-cat-wrap" style="display:none">
        <label class="form-label">نوع المحتوى</label>
        <select class="form-control" id="dstore-file-cat">
          <option value="books">📚 كتب PDF</option>
          <option value="apps">📱 تطبيقات Android APK</option>
          <option value="games">🎮 ألعاب ZIP</option>
...
window.ph45_saveStore = async function() {
  ...
  const storeType = document.getElementById('dstore-type')?.value || 'code';
  const fileCategory = storeType === 'file' ? (document.getElementById('dstore-file-cat')?.value || 'other') : null;
  ...
  const storeAddressId = null;
  const storeAddressName = null;
  const storeRegionName = null;
  ...
  showLoader();
  try {
    await fsAdd('digital_stores', { catId, regionId, name, desc, commission, tax, storeType, fileCategory, requireVendor, vendors, active, icon, order:0, hasAddress, storeAddressId, storeAddressName, storeRegionName });
    window.__ph45_pendingImg = null;
    await ph45_reloadDigitalData();
  ...
  openModal(`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="form-group">
        <label class="form-label">نوع المتجر</label>
        <select class="form-control" id="dstore-type" onchange="ph45_onStoreTypeChange(this.value)">
          <option value="code" ${s.storeType!=='file'?'selected':''}>🔑 أكواد رقمية</option>
          <option value="file" ${s.storeType==='file'?'selected':''}>📂 ملفات رقمية</option>
        </select>
      </div>
      <div class="form-group" id="dstore-file-cat-wrap" style="display:${s.storeType==='file'?'block':'none'}">
        <label class="form-label">نوع المحتوى</label>
        <select class="form-control" id="dstore-file-cat">
          <option value="books" ${s.fileCategory==='books'?'selected':''}>📚 كتب PDF</option>
          <option value="apps" ${s.fileCategory==='apps'?'selected':''}>📱 تطبيقات Android APK</option>
          <option value="games" ${s.fileCategory==='games'?'selected':''}>🎮 ألعاب ZIP</option>
          <option value="other" ${s.fileCategory==='other'||!s.fileCategory?'selected':''}>📦 ملفات أخرى</option>
        </select>
      </div>
    </div>
...
window.ph45_updateStore = async function(id) {
  ...
  if (!name) { toast('أدخل اسم المتجر', 'error'); return; }
  if (requireVendor && !vendors.length) { toast('يرجى اختيار مزود خدمة واحد على الأقل', 'error'); return; }
  const upd = { catId, regionId, name, desc, commission, tax, storeType, fileCategory, requireVendor, vendors, active, hasAddress, storeAddressId, storeAddressName, storeRegionName };
      ...
      ${prods.length ? prods.map(p => {
        ...
        <div style="background:var(--bg-card);border:1px solid var(--glass-border);border-radius:12px;padding:12px;margin-bottom:10px;display:flex;align-items:center;justify-content:space-between">
          <div>
            ...
            ${s.storeType === 'file' && p.fileName ? `<div style="font-size:11px;color:var(--text-muted);margin-top:4px">📂 ${escHtml(p.fileName)} (${ph45_formatBytes(p.fileSize)})</div>` : ''}
          </div>
          <div style="display:flex;align-items:center;gap:6px">
            ${s.storeType === 'file' ? `
              <button class="btn btn-sm" style="background:rgba(16,185,129,0.1);border:1px solid rgba(16,185,129,0.3);color:#10b981;display:flex;align-items:center;gap:4px" onclick="ph45_showUploadFileModal('${p.id}', '${storeId}')">
                📁 ${p.fileName ? 'تغيير الملف' : 'رفع ملف'}
              </button>
            ` : `
...
window.ph45_assignCodeToOrder = async function (orderId) {
  ...
  for (const item of order.items) {
    ...
    // Check if the store/product is file-based
    const store = (AppData.digitalStores || []).find(s => s.id === product.storeId);
    const isFileProduct = store && store.storeType === 'file';

    if (isFileProduct) {
      if (!product.fileUrl) {
        toast(`⚠️ المنتج ${product.name} لا يحتوي على ملف مرفق لتنزيله`, 'warning');
        continue;
      }
      try {
        await fsUpdate('orders', orderId, {
          downloadUrl: product.fileUrl,
          downloadFileName: product.fileName || 'file',
          downloadFileSize: product.fileSize || 0,
          digitalProductName: product.name
        });
        const localOrder = (AppData.orders || []).find(o => o.id === orderId);
        if (localOrder) {
          localOrder.downloadUrl = product.fileUrl;
          localOrder.downloadFileName = product.fileName || 'file';
          localOrder.downloadFileSize = product.fileSize || 0;
          localOrder.digitalProductName = product.name;
        }
        toast(`📂 تم تعيين ملف التنزيل لمنتج ${product.name}`, 'success');
      } catch(e) {
        console.error('Error assigning file to order', e);
      }
    } else {
...
window.ph45_uploadDigitalFile = function(productId, file, onProgress, onSuccess, onError) {
  const ref = firebase.storage().ref().child(`digital_files/${productId}/${file.name}`);
...
// UI for uploading files to product
window.ph45_showUploadFileModal = function(productId, storeId) {
  const p = (AppData.digitalProducts||[]).find(x => x.id === productId);
  if (!p) return;

  openModal(`
    <div class="modal-header">
      <h2 class="modal-title">📂 إدارة ملف المنتج — ${escHtml(p.name)}</h2>
      <button class="modal-close" onclick="closeModal()">✕</button>
    </div>

    <div style="background:var(--bg-secondary);padding:14px;border-radius:12px;border:1px solid var(--glass-border);margin-bottom:16px">
      <div style="font-weight:700;margin-bottom:6px">الملف الحالي:</div>
      ${p.fileName ? `
        <div style="display:flex;align-items:center;justify-content:space-between;background:var(--bg-card);padding:10px;border-radius:8px">
          <span style="font-family:monospace;font-size:13px">📂 ${escHtml(p.fileName)}</span>
          <span style="font-size:12px;color:var(--text-muted)">${ph45_formatBytes(p.fileSize)}</span>
        </div>
      ` : `
        <div style="color:var(--text-muted);font-size:13px">لم يتم رفع أي ملف بعد لهذا المنتج.</div>
      `}
    </div>

    <div style="background:var(--bg-secondary);border-radius:14px;padding:18px">
      <div style="font-weight:700;margin-bottom:14px;font-size:14px">⬆️ رفع ملف جديد</div>
      
      <div class="form-group">
        <label class="form-label">اختر الملف (PDF, APK, ZIP, etc.)</label>
        <input type="file" class="form-control" id="dprod-file-input">
      </div>

      <div id="dprod-upload-progress-wrap" style="display:none;margin-top:14px;margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:6px">
          <span>جارٍ الرفع...</span>
          <span id="dprod-upload-pct">0%</span>
        </div>
        <div style="width:100%;height:8px;background:var(--bg-card);border-radius:4px;overflow:hidden">
          <div id="dprod-upload-progress-bar" style="width:0%;height:100%;background:var(--primary);transition:width 0.1s"></div>
        </div>
      </div>

      <button class="btn btn-primary btn-block" style="margin-top:10px" onclick="ph45_startFileUpload('${productId}', '${storeId}')">💾 بدء الرفع وحفظ</button>
    </div>
  `);
};

window.ph45_startFileUpload = function(productId, storeId) {
  const fileInput = document.getElementById('dprod-file-input');
  const file = fileInput?.files[0];
  if (!file) { toast('يرجى اختيار ملف أولاً', 'error'); return; }

  const progressWrap = document.getElementById('dprod-upload-progress-wrap');
  const progressBar = document.getElementById('dprod-upload-progress-bar');
  const progressPct = document.getElementById('dprod-upload-pct');

  if (progressWrap) progressWrap.style.display = 'block';

  // Disable buttons
  const btn = document.querySelector('button[onclick*="ph45_startFileUpload"]');
  if (btn) btn.disabled = true;
  if (fileInput) fileInput.disabled = true;

  ph45_uploadDigitalFile(productId, file, 
    (progress) => {
    ...
    async (fileData) => {
      try {
        await fsUpdate('digital_products', productId, {
        ...
        await ph45_reloadDigitalData();
        toast('تم رفع الملف وحفظه بنجاح', 'success');
        closeModal();
        // Re-render admin UI or product list modal
        if (State._digBrowse && State._digBrowse.storeId) {
          render();
        } else {
          ph45_showManageProductsModal(storeId);
        }
      } catch(e) {
        toast('خطأ في حفظ البيانات: ' + e.message, 'error');
        if (btn) btn.disabled = false;
        if (fileInput) fileInput.disabled = false;
      }
    },
    (error) => {
      toast('خطأ في الرفع: ' + error.message, 'error');
      if (btn) btn.disabled = false;
      if (fileInput) fileInput.disabled = false;
    }
  );
...
// Customer helper for direct download
window.ph45_downloadFile = function(downloadUrl, fileName) {
  toast('⏳ بدء تحميل الملف...', 'info');
      ...
      ${stores.map(s => `
        <div onclick="navigate('stores', { digital: 'store', storeId: '${s.id}' })" style="background:var(--bg-card);border:1px solid var(--glass-border);border-radius:18px;padding:16px;display:flex;align-items:center;gap:14px;cursor:pointer;transition:all 0.2s">
          ...
          <div>
            ...
            ${window.ph47_getProductLocationHtml ? window.ph47_getProductLocationHtml(s.vendors) : ''}
...
function _ph45_renderProductCard(p, storeId) {
  ...
  const fallbackPrice = `<div class="ph-price-block ph-price-digital"><span class="ph-price-num">${(p.price || 0).toLocaleString('ar-YE')}</span><span class="ph-price-cur">ريال</span></div>`;
  ...
  const waND = (AppData.platformSettings && (AppData.platformSettings.whatsappNumberDigital || AppData.platformSettings.whatsappNumber) ? (AppData.platformSettings.whatsappNumberDigital || AppData.platformSettings.whatsappNumber) : '').replace(/\D/g, '');
  const waLink = waND ? '<a class="btn-wa-inquiry" style="border-radius:12px; display:inline-flex; align-items:center; justify-content:center; padding:10px; margin:0;" href="https://wa.me/' + waND + '?text=' + encodeURIComponent('أهلاً، أريد الاستفسار عن: ' + p.name) + '" target="_blank" rel="noopener" title="استفسار عبر واتساب"><svg viewBox="0 0 24 24" fill="currentColor" style="width:16px;height:16px"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>' : '';
  
  const store = (AppData.digitalStores || []).find(s => s.id === storeId);
  const locHtml = (store && window.ph47_getProductLocationHtml)
    ? window.ph47_getProductLocationHtml(store.vendors)
    : '';

  const isFile = store && store.storeType === 'file';
  const fileInfoHtml = isFile && p.fileName ? `
    <div style="font-size:12px;color:var(--text-secondary);background:var(--bg-secondary);padding:6px 10px;border-radius:8px;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;border:1px solid var(--glass-border)">
      <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:140px">📂 ${escHtml(p.fileName)}</span>
      <strong>${ph45_formatBytes(p.fileSize)}</strong>
    </div>` : '';
  ...
  return '<div style="background:var(--bg-card);border:1px solid ' + border + ';border-radius:16px;padding:16px;display:flex;flex-direction:column;justify-content:space-between;position:relative;overflow:hidden">'
    ...
    + '<button class="btn btn-primary" style="flex:1;border-radius:12px;font-weight:700;margin:0" onclick="ph45_addToCart(\'' + p.id + '\', \'' + storeId + '\')">' + btnLabel + '</button>'
    + waLink
    + '</div>'
    + '</div>';
}
