    ...
    <div class="service-body">
      ...
      <div class="service-footer">
        ...
        <div class="svc-card-actions">
          ${u?.role==='customer' ? `<button class="btn btn-primary btn-sm${isAvail ? '' : ' ia-btn-disabled'}" data-svc-cart-id="${s.id}" onclick="event.stopPropagation();${isAvail ? `typeof svc_addToCart==='function'?svc_addToCart('${s.id}'):bookService('${s.id}')` : `typeof toast==='function'&&toast('هذه الخدمة غير متاحة حالياً','warning')`}">${isAvail ? (activeOffer ? '🏷️ اطلب' : '🛒 أضف') : '🚫 غير متاح'}</button>` :
  ...
  return `<div id="app-content">
    ...
    <div class="svc-detail">
      ...
      <div class="svc-info">
        ...
        ${u?.role==='customer' ? `<button class="btn btn-primary btn-block btn-lg${isAvail ? '' : ' ia-btn-disabled'}" data-svc-cart-id="${s.id}" style="margin-top:16px" onclick="${isAvail ? `typeof svc_addToCart==='function'?svc_addToCart('${s.id}'):bookService('${s.id}')` : `typeof toast==='function'&&toast('هذه الخدمة غير متاحة حالياً','warning')`}">${isAvail ? '🛒 أضف للسلة' : '🚫 غير متاح'}</button>` :
