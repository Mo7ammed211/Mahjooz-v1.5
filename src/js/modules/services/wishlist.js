...
(function () {
  ...
  function getFavProducts(sortBy) {
    const ids = getFavIds();
    let items = ids
      .map(id => (AppData.storeProducts || []).find(p => p.id === id))
      .filter(Boolean);
    if (sortBy === 'price_asc')  items.sort((a,b) => (a.price||0) - (b.price||0));
    if (sortBy === 'price_desc') items.sort((a,b) => (b.price||0) - (a.price||0));
    if (sortBy === 'name')       items.sort((a,b) => (a.name||'').localeCompare(b.name||'', 'ar'));
  ...
  window.ExtraPages['favorites'] = function renderFavoritesPage() {
    ...
    if (!u || u.role === 'guest') {
      return `<div id="app-content"><div class="fav-empty">
        ...
        <p>سجّل دخولك لتتمكن من حفظ خدماتك ومنتجاتك المفضلة والوصول إليها في أي وقت</p>
    ...
    const allProducts = allIds.map(id => (AppData.storeProducts || []).find(p => p.id === id)).filter(Boolean);
    function sectionCount(key) {
      if (key === 'all') return allServices.length + allProducts.length;
      if (key === 'stores') return allProducts.length;
    ...
    const products = (activeSection === 'stores' || activeSection === 'all') ? getFavProducts(activeSort) : [];
      ...
      <div id="app-content">
        <div class="fav-page-hero">
          ...
          <div class="fav-page-hero-text">
            <h2>المفضلة فارغة</h2>
            <p>لم تقم بإضافة أي خدمات أو منتجات بعد</p>
          </div>
        </div>
        <div class="fav-empty">
          <div class="fav-empty-anim">🤍</div>
          <h3>أضف خدماتك ومنتجاتك المفضلة</h3>
          <p>تصفّح الخدمات والمتاجر واضغط على زر ❤️ لحفظها هنا وتجدها دائماً في متناول يدك</p>
    ...
    <div id="app-content">
      ...
      <div class="fav-page-hero">
        ...
        <div class="fav-page-hero-text">
          <h2>مفضلتي (${total})</h2>
          <p>جميع الخدمات والمنتجات التي أضفتها للمفضلة في مكان واحد</p>
        </div>
      ...
      <div class="fav-empty" style="padding:40px">
        ...
        <p>لم تضف خدمات أو منتجات من هذا القسم للمفضلة بعد</p>
        <button class="btn btn-secondary btn-sm" onclick="navigate('favorites',{favSection:'all',favSort:'${activeSort}'})">عرض الكل</button>
      </div>` : `

      <!-- Services Grid -->
      ${items.length > 0 ? `
      ${activeSection === 'all' ? '<h4 style="padding:0 16px;color:var(--text-muted);font-size:13px;margin-bottom:8px">🔧 الخدمات</h4>' : ''}
      <div class="fav-grid">
        ${items.map(s => `
      ...
      <!-- Products Grid (stores section) -->
      ${products.length > 0 ? `
      ${activeSection === 'all' ? '<h4 style="padding:0 16px;color:var(--text-muted);font-size:13px;margin-top:24px;margin-bottom:8px">🏪 منتجات المتاجر</h4>' : ''}
      <div class="ph43-product-grid" style="padding:0 16px 24px;">
        ${products.map(p => {
          const storeId = p.storeId || '';
          const cu = State.currentUser;
          return typeof window.ph43_renderProductCard === 'function'
            ? window.ph43_renderProductCard(p, storeId, cu)
            : '';
        }).join('')}
      </div>` : ''}
  ...
  window.favClearAll = function () {
    openModal(`
      ...
      <p style="color:var(--text-secondary);margin-bottom:24px;line-height:1.7">
        هل تريد مسح جميع الخدمات والمنتجات من مفضلتك؟<br>
